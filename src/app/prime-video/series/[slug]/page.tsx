...
import { Calendar, Globe, Headphones, Layers, Star } from "lucide-react";
...
import SeriesClient from "@/app/series/[slug]/SeriesClient";
...
export default async function PrimeSeriesDetailPage({ params }: Props) {
  const { slug } = await params;
  const data = await getPrimeSeries(slug);
  if (!data) notFound();
  ...
  return (
    <main className="min-h-screen bg-[#0F171E] pt-14 md:pt-20 pb-20 md:pb-0">
      ...
      <div className="max-w-[1800px] mx-auto px-4 md:px-8 -mt-28 md:-mt-44 relative z-10">
        <div className="flex flex-col md:flex-row gap-6 md:gap-10">
          ...
          <div className="flex-1 pt-4 md:pt-12">
            ...
            <div className="flex flex-wrap items-center gap-3 mb-3">
              ...
              {item.seasons && <span className="flex items-center gap-1 text-sm text-[#8197A4]"><Layers className="w-4 h-4" />{item.seasons.length} {item.seasons.length === 1 ? "Season" : "Seasons"}</span>}
            </div>
        ...
        {similar.length > 0 && (
          <div className="mt-16">
            <ContentRow title="More Prime Series" items={similar} />
          </div>
        )}
      </div>
    </main>
