...
export default async function SeriesDetailPage({ params }: Props) {
  ...
  const jsonLd = {
    ...
    seasons: item.seasons?.map((s: { seasonNumber: number; episodes: { episodeNumber: number; episodeTitle: string }[] }) => ({
      "@type": "TVSeason",
      seasonNumber: s.seasonNumber,
      episode: s.episodes?.map((e: { episodeNumber: number; episodeTitle: string }) => ({
        "@type": "TVEpisode",
    ...
    <>
      ...
      <main className="min-h-screen pt-16 pb-20 md:pb-0">
        ...
        <div className="max-w-[1800px] mx-auto px-4 md:px-8 -mt-32 md:-mt-48 relative z-10">
          ...
          {similar.length > 0 && (
            <div className="mt-16">
              <ContentRow title="Similar Series" items={similar} />
            </div>
          )}
        </div>
      </main>
